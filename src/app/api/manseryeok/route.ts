...
${focusArea ? `6. **중요: "${focusArea}" 주제에만 집중하세요. 다른 분야는 언급하지 마세요. 이 주제를 깊이 있게 파고들어주세요.` : ""}

## 형식
- **굵은 소제목**으로 구분 (이모지 + 주제)
- 각 소제목 아래에 반드시 2줄 이상의 내용을 작성할 것 (빈 섹션 금지)
${focusArea ? `- "${focusArea}" 관련 소주제 3~4개로 구성` : `- 주제별로 구성 (성격, 연애, 직업, 올해운세 등)`}
- 마지막에 **🔮 종합 결론 & 실전 조언** 필수`
