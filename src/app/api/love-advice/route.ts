...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!uid) return NextResponse.json({ advice: "⚠️ 로그인이 필요한 기능입니다." }, { status: 401 })
    const { checkRateLimit } = await import("@/lib/rate-limit")
    if (!checkRateLimit(uid, 5, 60000)) return NextResponse.json({ advice: "⚠️ 너무 많은 요청입니다. 1분 후 다시 시도해주세요." }, { status: 429 })
